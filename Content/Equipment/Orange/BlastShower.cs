...
namespace WellRoundedBalance.Equipment.Orange
{
    public class BlastShower : EquipmentBase<BlastShower>
    {
        ...
        {
            IL.RoR2.CleanseSystem.CleanseBodyServer_CharacterBody_refCleanseArgs += ChangeRemovalRange;
        ...
        private void ChangeRemovalRange(ILContext il)
        {
            ...
            if (c.TryGotoNext(MoveType.Before,
                x => x.MatchLdsfld(typeof(CleanseSystem), nameof(CleanseSystem.defaultProjectileEraseRadius))))
            {
                c.Index++;
                c.EmitDelegate<Func<float, float>>((x) => {
                    return projectileRemovalRange;
                });
            }
            else
            {
