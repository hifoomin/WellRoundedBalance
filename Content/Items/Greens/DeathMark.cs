...
namespace WellRoundedBalance.Items.Greens
{
    public class DeathMark : ItemBase
    {
        ...
        public override string DescText => "Enemies with <style=cIsDamage>2</style> or more debuffs are <style=cIsDamage>marked for death</style>, increasing damage taken by <style=cIsDamage>10%</style> <style=cStack>(+5% per stack)</style> per debuff from all sources for <style=cIsUtility>7</style> <style=cStack>(+7 per stack)</style> seconds .";
                ...
                ))
            {
                ...
                c.EmitDelegate<Func<HealthComponent, CharacterBody, float>>((self, attacker) =>
                {
                    if (attacker.master.inventory)
                    {
                        ...
                        float damageBonus = debuffCount * 0.1f;
                        if (DeathMarkCount > 0)
                        {
                            return 1f + damageBonus + (0.05f * damageBonus * ((float)DeathMarkCount - 1f));
                        }
