            ...
            if (damageReport.attacker && damageReport.victim)
            {
                ...
                if (attackerBody)
                {
                    ...
                    if (inventory && NetworkServer.active && attackerBody.healthComponent)
                        attackerBody.healthComponent.AddBarrier(attackerBody.healthComponent.combinedHealth * StackAmount(percentBarrierGain, percentBarrierGainStack, inventory.GetItemCountEffective(RoR2Content.Items.BarrierOnKill), percentBarrierGainIsHyperbolic));
                }
            }
        }
